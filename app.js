...
const adminPassword = "aalmwt10"; // كلمة مرور الإدارة
...
async function register() {
  ...
  if (signUpErr) {
    showMsg("خطأ في التسجيل: " + signUpErr.message);
    return;
  }
  ...
  if (insertErr) {
    showMsg("خطأ في حفظ المستخدم: " + insertErr.message);
    return;
  }

  // إنشاء 25 مهمة تلقائياً
  let deposit = 10;
  const tasksToInsert = [];
  for (let i = 1; i <= 25; i++) {
    tasksToInsert.push({
...
async function login() {
  ...
  if (!email || !pass) {
    showMsg("املأ البريد وكلمة المرور");
    return;
  }
  ...
  if (error) {
    showMsg("خطأ في تسجيل الدخول: " + error.message);
    return;
  }
  ...
  if (userErr) {
    showMsg("خطأ جلب بيانات المستخدم: " + userErr.message);
    return;
  }

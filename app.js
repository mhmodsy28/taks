...
async function login() {
  ...
  if (error) { showMsg("خطأ تسجيل الدخول: " + error.message); return; }
  ...
  document.getElementById("app").innerHTML = `
    <div class="container">
      ...
      <div class="account-box">
        ...
        <p>الرصيد: <b id="accBalance">${currentUser.balance}</b>$</p>
...
async function executeTask(taskId, reward) {
  const { data: trow } = await supabaseClient.from("tasks").select("*").eq("id", taskId).single();
  if (!trow || !trow.is_open) { showMsg("المهمة مغلقة"); return; }

  const newBalance = (currentUser.balance||0)+reward;
  await supabaseClient.from("users").update({ balance: newBalance }).eq("id", currentUser.uid);
...
supabaseClient.auth.onAuthStateChange(async (event, session)=>{
  if(session && session.user){
    ...
    const { data: userRow } = await supabaseClient.from("users").select("*").eq("id", uid).single();
